...
def getCutoff(cell_barcode_counts,
                           expect_cells=False,
                           cell_number=False,
			   expect_count=False,
    ...
    if plotfile_prefix:
        ...
        if threshold > 0:
            ...
            colours = [CB_color_cycle[0] for x in range(0, len(counts))]
        # make densitny plot
        fig = plt.figure()
        fig1 = fig.add_subplot(111)
        fig1.scatter(x=range(1, len(counts)+1), y=counts,
                     c=colours, s=10, linewidths=0)
        fig1.loglog()
        fig1.set_xlim(0, len(counts)*1.25)
        fig1.set_xlabel('Barcode index')
        fig1.set_ylabel('Count')
        fig1.set_title("%s" % plotfile_prefix)
        if cell_number:
            fig1.axvline(x=cell_number, ls="dashed", color=CB_color_cycle[0])
            lgd = fig1.legend(bbox_to_anchor=(1.05, 1), loc=2, borderaxespad=0.,
                              handles=[user_line],
                              title="Cell threshold")
        elif isinstance(expect_count, int):
            fig1.axvline(x=len(counts_thresh), ls="dashed", color=CB_color_cycle[0])
            lgd = fig1.legend(bbox_to_anchor=(1.05, 1), loc=2, borderaxespad=0.,
                              handles=[user_line],
                              title="Cell threshold")
        fig.savefig("%s_cell_barcode_count_cutoff.png" % plotfile_prefix,
...
def getCellWhitelist(cell_barcode_counts,
    ...
    elif knee_method == "None":
        cell_whitelist = getCutoff(cell_barcode_counts, expect_cells, cell_number, expect_count, plotfile_prefix)
